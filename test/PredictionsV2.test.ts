...
const PancakePredictionV2 = artifacts.require('PredictionsV2.sol');
...
const DECIMALS = 8; // Pyth default for SEI/USD
...
contract(
  'PredictionsV2.sol',
  ([operator, admin, owner, bullUser1, bullUser2, bearUser1]) => {
    ...
    it('Should start genesis rounds (round 1, round 2, round 3)', async () => {
      ...
      // Start round 2
      assert.equal(
        (await prediction.rounds(2)).startTimestamp,
        currentTimestamp
      );
      assert.equal(
    ...
    it('Should record data and user bets', async () => {
      ...
      await prediction.betBull(currentEpoch, {
        from: bullUser1,
        value: ether('1.1'),
      }); // 1.1 BNB
      await prediction.betBull(currentEpoch, {
      ...
      await prediction.betBear(currentEpoch, {
        from: bearUser1,
        value: ether('1.4'),
      }); // 1.4 BNB
      ...
      assert.equal(
        (await prediction.ledger(1, bullUser1)).position,
        Position.Bull
      );
      assert.equal(
        (await prediction.ledger(1, bullUser1)).amount,
        ether('1.1').toString()
      );
      assert.equal(
        (await prediction.ledger(1, bullUser2)).position,
        Position.Bull
      );
      assert.equal(
        (await prediction.ledger(1, bullUser2)).amount,
        ether('1.2').toString()
      );
      assert.equal(
        (await prediction.ledger(1, bearUser1)).position,
        Position.Bear
      );
      assert.equal(
        (await prediction.ledger(1, bearUser1)).amount,
        ether('1.4').toString()
      );
      ...
      assert.equal(
        (await prediction.ledger(2, bullUser1)).position,
        Position.Bull
      );
      assert.equal(
        (await prediction.ledger(2, bullUser1)).amount,
        ether('2.1').toString()
      );
      assert.equal(
        (await prediction.ledger(2, bullUser2)).position,
        Position.Bull
      );
      assert.equal(
        (await prediction.ledger(2, bullUser2)).amount,
        ether('2.2').toString()
      );
      assert.equal(
        (await prediction.ledger(2, bearUser1)).position,
        Position.Bear
      );
      assert.equal(
        (await prediction.ledger(2, bearUser1)).amount,
        ether('2.4').toString()
      );
      assertBNArray(
        (await prediction.getUserRounds(bullUser1, 0, 2))[0],
        [1, 2]
      );
      assertBNArray(
        (await prediction.getUserRounds(bullUser2, 0, 2))[0],
        [1, 2]
      );
      assertBNArray(
        (await prediction.getUserRounds(bearUser1, 0, 2))[0],
        [1, 2]
      );
      ...
      assert.equal(
        (await prediction.ledger(3, bullUser1)).position,
        Position.Bull
      );
      assert.equal(
        (await prediction.ledger(3, bullUser1)).amount,
        ether('3.1').toString()
      );
      assert.equal(
        (await prediction.ledger(3, bullUser2)).position,
        Position.Bull
      );
      assert.equal(
        (await prediction.ledger(3, bullUser2)).amount,
        ether('3.2').toString()
      );
      assert.equal(
        (await prediction.ledger(3, bearUser1)).position,
        Position.Bear
      );
      assert.equal(
        (await prediction.ledger(3, bearUser1)).amount,
        ether('3.4').toString()
      );
      assertBNArray(
        (await prediction.getUserRounds(bullUser1, 0, 3))[0],
        [1, 2, 3]
      );
      assertBNArray(
        (await prediction.getUserRounds(bullUser2, 0, 3))[0],
        [1, 2, 3]
      );
      assertBNArray(
        (await prediction.getUserRounds(bearUser1, 0, 3))[0],
        [1, 2, 3]
      );
      ...
      assert.equal(
        (await prediction.ledger(4, bullUser1)).position,
        Position.Bull
      );
      assert.equal(
        (await prediction.ledger(4, bullUser1)).amount,
        ether('4.1').toString()
      );
      assert.equal(
        (await prediction.ledger(4, bullUser2)).position,
        Position.Bull
      );
      assert.equal(
        (await prediction.ledger(4, bullUser2)).amount,
        ether('4.2').toString()
      );
      assert.equal(
        (await prediction.ledger(4, bearUser1)).position,
        Position.Bear
      );
      assert.equal(
        (await prediction.ledger(4, bearUser1)).amount,
        ether('4.4').toString()
      );
      assertBNArray(
        (await prediction.getUserRounds(bullUser1, 0, 4))[0],
        [1, 2, 3, 4]
      );
      assertBNArray(
        (await prediction.getUserRounds(bullUser2, 0, 4))[0],
        [1, 2, 3, 4]
      );
      assertBNArray(
        (await prediction.getUserRounds(bearUser1, 0, 4))[0],
        [1, 2, 3, 4]
      );
    ...
    it('Should not allow multiple bets', async () => {
      ...
      await expectRevert(
        prediction.betBull(currentEpoch, {
          from: bullUser1,
          value: ether('1'),
        }),
      ...
      await expectRevert(
        prediction.betBear(currentEpoch, {
          from: bullUser1,
          value: ether('1'),
        }),
        'Can only bet once per round'
    ...
    it('Should claim rewards', async () => {
      ...
      await expectRevert(
        prediction.claim([1], { from: bullUser1 }),
        'Round has not ended'
      );
      await expectRevert(
        prediction.claim([1], { from: bullUser2 }),
        'Round has not ended'
      );
      await expectRevert(
        prediction.claim([1], { from: bearUser1 }),
        'Round has not ended'
      );
      await expectRevert(
        prediction.claim([2], { from: bullUser1 }),
        'Round has not started'
      );
      await expectRevert(
        prediction.claim([2], { from: bullUser2 }),
        'Round has not started'
      );
      await expectRevert(
        prediction.claim([2], { from: bearUser1 }),
        'Round has not started'
      );
      ...
      assert.equal(
        ...
        ether('2.1').sub(calcGasCost(gasUsed)).toString()
      );
      ...
      assert.equal(
        ...
        ether('4.2').sub(calcGasCost(gasUsed)).toString()
      );
      ...
      assert.equal(
        ...
        ether('60.3').sub(calcGasCost(gasUsed)).toString()
      );
    ...
    it('Should multi claim rewards', async () => {
      ...
      await expectRevert(
        prediction.claim([2, 2], { from: bullUser1 }),
        'Not eligible for claim'
      );
      await expectRevert(
        prediction.claim([1, 1], { from: bullUser1 }),
        'Not eligible for claim'
      );
      ...
      assert.equal(
        ...
        ether('29.448837209302325581').toString()
      );
      ...
      assert.equal(
        tx.logs[1].args.amount.toString(),
        ether('30.851162790697674418').toString()
      );
      ...
      await expectRevert(
        prediction.claim([1, 2], { from: bullUser1 }),
        'Not eligible for claim'
      );
      await expectRevert(
        prediction.claim([2, 1], { from: bullUser1 }),
        'Not eligible for claim'
      );
      await expectRevert(
        prediction.claim([1, 2], { from: bullUser2 }),
        'Not eligible for claim'
      );
      await expectRevert(
        prediction.claim([2, 1], { from: bullUser2 }),
        'Not eligible for claim'
      );
      await expectRevert(
        prediction.claim([1], { from: bearUser1 }),
    ...
    it('Should claim treasury rewards', async () => {
      ...
      assert.equal(
        ...
        ether('0.7').sub(calcGasCost(gasUsed)).toString()
      );
      ...
      assert.equal(
        (await adminTracker.delta()).toString(),
        ether('16.4').sub(calcGasCost(gasUsed)).toString()
      );
      assert.equal(await prediction.treasuryAmount(), 0); // Empty
    ...
    it('Admin/Owner function work as expected', async () => {
      ...
      let tx = await prediction.setBufferAndIntervalSeconds('50', '100', {
        from: admin,
      });

      expectEvent(tx, 'NewBufferAndIntervalSeconds', {
        bufferSeconds: '50',
        intervalSeconds: '100',
      });
      ...
      await expectRevert(
        prediction.setMinBetAmount('0', { from: admin }),
      ...
      await expectRevert(
        prediction.setOperator(constants.ZERO_ADDRESS, { from: admin }),
      ...
      await expectRevert(
        prediction.setOracle(constants.ZERO_ADDRESS, { from: admin }),
        'Cannot be zero address'
      );
      ...
      // EOA
      await expectRevert(
        prediction.setOracle(admin, { from: admin }),
      ...
      expectEvent(tx, 'NewOracleUpdateAllowance', {
        oracleUpdateAllowance: '30',
      });
      ...
      await expectRevert(
        prediction.setTreasuryFee('3000', { from: admin }),
      ...
      await expectRevert(
        prediction.setAdmin(constants.ZERO_ADDRESS, { from: owner }),
        'Cannot be zero address'
      );
    });
    ...
    it('Should reject operator functions when not operator', async () => {
      await expectRevert(
        prediction.genesisLockRound({ from: admin }),
        'Not operator'
      );
      await expectRevert(
        prediction.genesisStartRound({ from: admin }),
        'Not operator'
      );
      await expectRevert(
        prediction.executeRound({ from: admin }),
        'Not operator'
      );
    });
    ...
    it('Should reject admin/owner functions when not admin/owner', async () => {
      await expectRevert(
        prediction.claimTreasury({ from: bullUser1 }),
        'Not admin'
      );
      await expectRevert(
        prediction.pause({ from: bullUser1 }),
        'Not operator/admin'
      );
      await prediction.pause({ from: admin });
      await expectRevert(prediction.unpause({ from: bullUser1 }), 'Not admin');
      await expectRevert(
        prediction.setBufferAndIntervalSeconds('50', '100', {
          from: bullUser1,
        }),
        'Not admin'
      );
      await expectRevert(
        prediction.setMinBetAmount('0', { from: bullUser1 }),
        'Not admin'
      );
      await expectRevert(
        prediction.setOperator(bearUser1, { from: bullUser1 }),
        'Not admin'
      );
      await expectRevert(
        prediction.setOracle(bearUser1, { from: bullUser1 }),
        'Not admin'
      );
      await expectRevert(
        prediction.setOracleUpdateAllowance('0', { from: bullUser1 }),
        'Not admin'
      );
      await expectRevert(
        prediction.setTreasuryFee('100', { from: bullUser1 }),
        'Not admin'
      );
      await expectRevert(prediction.unpause({ from: bullUser1 }), 'Not admin');
      await prediction.unpause({ from: admin });
      await expectRevert(
        prediction.setAdmin(admin, { from: admin }),
        'Ownable: caller is not the owner'
      );
      await expectRevert(
        prediction.setAdmin(bullUser1, { from: bullUser1 }),
        'Ownable: caller is not the owner'
      );
    });
    ...
    it('Should reject admin/owner functions when not paused', async () => {
      await expectRevert(
        prediction.setBufferAndIntervalSeconds('50', '100', { from: admin }),
        'Pausable: not paused'
      );
      await expectRevert(
        prediction.setMinBetAmount('0', { from: admin }),
      ...
      await expectRevert(
        prediction.setOracle(bearUser1, { from: admin }),
        'Pausable: not paused'
      );
      await expectRevert(
        prediction.setOracleUpdateAllowance('0', { from: admin }),
        'Pausable: not paused'
      );
      await expectRevert(
        prediction.setTreasuryFee('100', { from: admin }),
        'Pausable: not paused'
      );
      await expectRevert(
        prediction.unpause({ from: admin }),
        'Pausable: not paused'
      );
    });
    ...
    it('Rejections for bet bulls/bears work as expected', async () => {
      // Epoch 0
      await expectRevert(
        prediction.betBull('0', { from: bullUser1, value: ether('1') }),
        'Round not bettable'
      );
      await expectRevert(
        prediction.betBear('0', { from: bullUser1, value: ether('1') }),
        'Round not bettable'
      );
      await expectRevert(
        prediction.betBull('1', { from: bullUser1, value: ether('1') }),
        'Bet is too early/late'
      );
      await expectRevert(
        prediction.betBear('1', { from: bullUser1, value: ether('1') }),
        'Bet is too early/late'
      );
      ...
      currentEpoch = await prediction.currentEpoch();
      await expectRevert(
        prediction.betBull('2', { from: bullUser1, value: ether('1') }),
        'Bet is too early/late'
      );
      await expectRevert(
        prediction.betBear('2', { from: bullUser1, value: ether('1') }),
        'Bet is too early/late'
      );
    ...
    it('Should paginate user rounds', async () => {
      ...
      assertBNArray(
        (await prediction.getUserRounds(bullUser1, 0, 5))[0],
        [1, 2, 3, 4, 5]
      );
