...
describe("PredictionV4", () => {
  let operator, admin, owner, bullUser1, bullUser2, bearUser1, users;
  ...
  // Initialize users
  before(async () => {
    [operator, admin, owner, bullUser1, bullUser2, bearUser1, ...users] =
  ...
  beforeEach(async () => {
    ...
    const predictionV4ImplmentationFactory =
      await ethers.getContractFactory("PredictionV4");
    const predictionV4Implementation =
      await predictionV4ImplmentationFactory.deploy();
    await predictionV4Implementation.deployed();

    await predictionsFactory
      .connect(owner)
      .setImplementationPredictionV4(predictionV4Implementation.address);

    const predictionV4CreationTx = await predictionsFactory
      .connect(owner)
      .deployPredictionV4(
        admin.address,
        operator.address,
    ...
    const predictionV4TxReceipt = await predictionV4CreationTx.wait();

    prediction = await ethers.getContractAt(
      "PredictionV4",
      predictionV4TxReceipt.logs[0].address,
    );
  });
  ...
  it("Should record data and user bets", async () => {
    ...
    await prediction
      .connect(bullUser1)
      .betBull(currentRound, { value: parseEther("1.1").toString() }); // 1.1 SEI
    await prediction
      .connect(bullUser2)
      .betBull(currentRound, { value: parseEther("1.2").toString() }); // 1.2 SEI
    await prediction
      .connect(bearUser1)
      .betBear(currentRound, { value: parseEther("1.4").toString() }); // 1.4 SEI
  ...
  it("Should not allow multiple bets", async () => {
    ...
    await prediction
      .connect(bullUser1)
      .betBull(currentRound, { value: parseEther("1").toString() }); // Success
    await expect(
      prediction
        .connect(bullUser1)
        .betBull(currentRound, { value: parseEther("1").toString() }),
    ).to.be.revertedWith("Can only bet once per round");
    await expect(
      prediction
        .connect(bullUser1)
        .betBear(currentRound, { value: parseEther("1").toString() }),
    ).to.be.revertedWith("Can only bet once per round");
    await prediction
      .connect(bearUser1)
      .betBear(currentRound, { value: parseEther("1").toString() }); // Success
    await expect(
      prediction
        .connect(bearUser1)
        .betBull(currentRound, { value: parseEther("1").toString() }),
  ...
  it("Should not allow bets lesser than minimum bet amount", async () => {
    ...
    await expect(
      prediction
        .connect(bullUser1)
        .betBull(currentRound, { value: parseEther("0.5").toString() }),
  ...
  it("Should fail to claim rewards due to round not ended", async () => {
    ...
    await prediction
      .connect(bullUser1)
      .betBull(currentRound, { value: parseEther("1").toString() }); // 1 SEI
    await prediction
      .connect(bullUser2)
      .betBull(currentRound, { value: parseEther("2").toString() }); // 2 SEI
    await prediction
      .connect(bearUser1)
      .betBear(currentRound, { value: parseEther("4").toString() }); // 4 SEI
